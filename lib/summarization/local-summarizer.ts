...
export class GeminiSummarizer implements LocalSummarizer {
  ...
  async summarize(text: string, _maxLength = 200): Promise<string | null> {
    ...
    const systemInstruction = [
      ...
      '다음 규칙을 반드시 따르십시오.',
      '1. 핵심 포인트 3개를 불릿(✦) 형식으로 작성한다. 각 불릿은 반드시 완전한 서술형 문장 하나로 구성한다.',
      '2. 구어체, 반복 표현, 불필요한 세부 예시는 제거한다. 같은 내용을 다른 표현으로 반복하지 않는다. 각 불릿은 서로 다른 관점이나 새로운 정보를 담아야 한다.',
      '3. 첫 번째 불릿은 주제와 핵심 상황을, 두 번째는 주요 근거/데이터를, 세 번째는 결론/시사점을 담는다.',
      '4. 객관적이고 간결한 문어체 문체를 사용한다.',
      '5. 반드시 한국어로만 답한다.',
      '6. 출력 형식: 줄바꿈으로 구분된 불릿 3개만 출력하고, 그 외 텍스트는 일절 포함하지 않는다.',
    ].join(' ')
